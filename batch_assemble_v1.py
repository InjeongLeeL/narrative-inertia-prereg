#!/usr/bin/env python3
# batch_assemble_v1.py — 설계 E 사전등록 수정안 v3.12 §정정4-1(2) 동결 산출물.
...
#   V 유래 few-shot 앵커 14건 = **미승계**(배치 컴팩트성 + V 누출 차단; 코드북 약식 rubric은 보존). 상세 공시 = prompt_template_v2.0_batch.md.
# 배치 규칙(v3.12 §4-2): 청크 내 dyad_id canonical 오름차순·고정 크기 50(마지막 잔여 <50), dyad_id echo 필수, 위치매칭 금지.
# 사용: python batch_assemble_v1.py --chunk out/arm_E/chunks/chunk01.csv --out out/arm_E/batches/chunk01_batches.jsonl [--map <map.csv>]
...
# ---- 약식 결정트리 + 코드북 rubric(v1.4 준거, 앵커 미승계·condensed) ----
